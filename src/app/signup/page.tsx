...
import { ShieldCheck, User, Mail, Lock, Eye, EyeOff, Loader2, AlertCircle } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const getStrengthMeta = (score: number) => {
    switch (score) {
      case 0:
        return { label: 'None', colorClass: 'bg-zinc-800', textClass: 'text-zinc-500' }
      case 1:
        return { label: 'Weak', colorClass: 'bg-rose-500', textClass: 'text-rose-400' }
      case 2:
        return { label: 'Fair', colorClass: 'bg-amber-500', textClass: 'text-amber-400' }
      case 3:
        return { label: 'Good', colorClass: 'bg-yellow-400', textClass: 'text-yellow-400' }
      case 4:
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-zinc-400">
            Already registered?{' '}
            <Link href="/login" id="link_to_login" className="font-medium text-emerald-400 hover:underline">
              Log in here
            </Link>
          </p>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-8 backdrop-blur-xl shadow-xl shadow-black/50">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
            ...
            {/* Full Name Input */}
            <div>
              <label htmlFor="input_fullname" className="block text-xs font-semibold uppercase tracking-wider text-zinc-400">
                Full Name
              </label>
              ...
              {errors.name && (
                <p id="error_fullname" className="mt-1.5 text-xs text-rose-400">
                  {errors.name.message}
                </p>
              )}
            </div>
              ...
              {passwordValue.length > 0 && (
                <div className="mt-3">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={`font-semibold ${strengthMeta.textClass}`}>{strengthMeta.label}</span>
                  </div>
                  {/* Progress bars */}
                    ...
                    {[1, 2, 3, 4].map((step) => (
                      <div
                        ...
                        className={`h-1 rounded-full transition-colors duration-300 ${
                          strengthScore >= step ? strengthMeta.colorClass : 'bg-zinc-800'
                        }`}
                      />
                    ))}
                  </div>
